...
import { HeroDatabase, HeroDocument } from "./types";
...
export const Heroes = () => {
  const { db } = useContext<{ db: HeroDatabase | null }>(AppContext);
  ...
  const addHero = async () => {
    ...
    if (!db || !db[HeroesCollectionName]) return;
    await db[HeroesCollectionName].insert({ name, color });
    setName("");
  };

  const removeHero = async (hero: HeroDocument) => {
    Alert.alert(
      "Delete hero?",
  ...
  return (
    <View style={styles.topContainer}>
      ...
      <Text style={styles.subtitle}>RxDB Premium SQLite Plugin example</Text>
